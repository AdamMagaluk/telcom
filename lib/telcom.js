...
TelcomClient.prototype.expressValidate = function(){
  ...
  return function(req,res,next){
    ...
    req._telcomRequestUrl = req.protocol + "://" + req.header('host') + origUrl;
    req._telcomRequestUrlNoQuery = req.protocol + "://" + req.header('host') + URL.parse(req.url).pathname;
...
TelcomClient.prototype.onSms = function(handler){
  ...
  return function(req,res,next){
    if(!req._telcomValidated){
      res.statusCode = 403;
      res.send('Invalid request from provider ' + self.options.provider)
    ...
    if(typeof sms === 'string'){
      res.send(400)
      res.send('Invalid requrest from provider ' + self.options.provider + ' '+ sms)
      if(typeof next === 'function')
        return next();
    }
    ...
    if(typeof next === 'function'){
      return handler(req,res,next,sms);
    }else{
